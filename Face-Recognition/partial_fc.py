...
class PartialFC(torch.nn.Module):
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(self.fp16):
            #Embeddings and Proxies/Weights are Normalized Right Here
            #norm_embeddings = normalize(embeddings)
            #norm_weight_activated = normalize(self.weight_activated)
            embeddings = 1.0*embeddings#normalize(embeddings)
            weight_activated = 1.0*self.weight_activated#normalize(self.weight_activated)

            #logits = linear(norm_embeddings, norm_weight_activated)
        if self.fp16:
            #norm_embeddings = norm_embeddings.float()
            #norm_weight_activated = norm_weight_activated.float()
            embeddings = embeddings.float()
            weight_activated = weight_activated.float()
            #logits = logits.float()
        #logits = logits.clamp(-1, 1)
        logits = self.margin_softmax(embeddings, weight_activated, labels)
...
class DistCrossEntropyFunc(torch.autograd.Function):
    ...
    def forward(ctx, logits: torch.Tensor, label: torch.Tensor):
        ...
        Logits = logits.clone()
        #max_logits = torch.median(Logits.detach(), dim=1, keepdim=True)[0]
        max_logits = torch.max(Logits.detach(), dim=1, keepdim=True)[0]
